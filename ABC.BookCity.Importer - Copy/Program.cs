...
while (true)
{
    ...
    if (input.ToLower() == "h")
    {
        ...
        {
            schemaPath = @"H:\Developer.BookCity\ABC.BookCity\DataImports\hathitrust\create_hathitrust_tables.sql";
        }
        ...
        // Now run the loader
        var loader = new ABC.BookCity.MariaDB.HathiTrustLoader(connectionString, hathiPath, batchSize: 5000);
    ...
    if (input.ToLower() == "w")
    {
        ...
        // Run the loader
        var wcLoader = new ABC.BookCity.MariaDB.WorldCatLoader(connectionString, wcPath, batchSize: 5000);
    ...
    if (input.ToLower() == "j")
    {
        ...
        if (!File.Exists(schemaPath))
        {
            schemaPath = @"H:\Developer.BookCity\ABC.BookCity\DataImports\worldcat\create_worldcat_records_table.sql";
        }
        ...
        // Run the loader
        var jsonLoader = new ABC.BookCity.MariaDB.WorldCatJsonLoader(connectionString, jsonPath, batchSize: 2000);
    ...
    if (input.ToLower() == "c")
    {
        ...
        // Run the loader
        var codesLoader = new ABC.BookCity.MariaDB.AarecordsCodesLoader(connectionString, codesPath, batchSize: 10000);
    ...
    switch (download.Type)
    {
        case TorrentDownloadManager.DownloadType.JsonlSeekableZst:
            if (download.TorrentName.Contains("worldcat"))
            {
                ...
                var loader = new ABC.BookCity.MariaDB.WorldCatJsonLoader(
                    connectionString, 
