...
Console.WriteLine("║         From staging to published                            ║");
Console.WriteLine("╚══════════════════════════════════════════════════════════════╝");
Console.WriteLine();


string sourceConnectionString = "Server=localhost;Port=3307;User=allthethings;Password=password;Database=libgen_new;AllowLoadLocalInfile=true;AllowZeroDateTime=true;ConvertZeroDateTime=false;";

string targetConnectionString = "Server=localhost;Port=3306;User=root;Password=password;Database=allthethings;AllowLoadLocalInfile=true;AllowZeroDateTime=true;ConvertZeroDateTime=false;";

string stateFile = @"H:\_DOCKER-DATA\sync_state.json";
...
// JsonSerializerContext for AOT-compatible serialization
[JsonSerializable(typeof(Dictionary<string, TableSyncProgress>))]
[JsonSerializable(typeof(TableSyncProgress))]
...
public class SyncState
{
    ...
    private void Load()
    {
        try
        {
            if (File.Exists(_stateFile))
            {
                ...
                var loaded = JsonSerializer.Deserialize(json, SyncJsonContext.Default.DictionaryStringTableSyncProgress);
                if (loaded != null)
                {
                    _progress = loaded;
                }
    ...
    private void Save()
    {
        try
        {
            ...
            var json = JsonSerializer.Serialize(_progress, SyncJsonContext.Default.DictionaryStringTableSyncProgress);
